...
module.exports = function validateFunction(options) {
    ...
    function validateFunctionStop(options) {
        ...
        if (isZoomAndPropertyFunction) {
            ...
            if (value[0].value === undefined && functionType.indexOf('dragonfly') === -1) {
    ...
    function validateStopDomainValue(options) {
        ...
        } else if (type !== stopKeyType && functionType !== 'dragonfly-interval' && functionType !== 'dragonfly-categorical') { //!
        ...
        if (type !== 'number' && type !== 'string' && functionType !== 'dragonfly-interval' && functionType !== 'dragonfly-categorical') { //!
            return [new ValidationError(options.key, options.value, 'property value must be a number or string')];
        }

        if (type === 'string' && value !== 'insufficient' && functionType === 'dragonfly-interval') { //!
            return [new ValidationError(options.key, options.value, 'property value must be a number, "insufficient" or undefined')];
        }

        if (type !== 'number' && type !== 'undefined' && type !== 'string' && functionType === 'dragonfly-interval') { //!
            return [new ValidationError(options.key, options.value, 'property value must be a number, "insufficient" or undefined')];
        }

        if (type !== 'number' && functionType !== 'categorical' && functionType !== 'dragonfly-interval' && functionType !== 'dragonfly-categorical') { //!
